...
entailment_constructor <- torch::dataset(
  ...
  initialize = function(premises, hypotheses, labels = NULL, n_tokens = 128) {
    tokenized_text <- tokenize_bert(
      premises,
      hypotheses,
      n_tokens = n_tokens
    )
    ...
    self$labels = torch::torch_tensor(as.integer(labels))
  },
...
We use that dataset constructor to process the train, matched, and mismatched datasets.

```{r datasets}
train_ds <- entailment_constructor(
  premises = mnli_tibbles$train$sentence1,
  hypotheses = mnli_tibbles$train$sentence2,
  labels = mnli_tibbles$train$gold_label
)
test_matched_ds <- entailment_constructor(
  premises = mnli_tibbles$dev_matched$sentence1,
  hypotheses = mnli_tibbles$dev_matched$sentence2,
  labels = mnli_tibbles$dev_matched$gold_label
)
test_mismatched_ds <- entailment_constructor(
  premises = mnli_tibbles$dev_mismatched$sentence1,
  hypotheses = mnli_tibbles$dev_mismatched$sentence2,
  labels = mnli_tibbles$dev_mismatched$gold_label
)
```
...
entailment_classifier <- torch::nn_module(
  ...
  initialize = function(model_name = "bert_tiny_uncased") {
    embedding_size <- config_bert(model_name, "embedding_size")
...
#>   <chr>    <chr>          <dbl>
#> 1 accuracy multiclass     0.651

predictions_mismatched <- fitted %>% 
  predict(test_mismatched_ds) %>% 
  torch::nnf_softmax(2) %>% 
  torch::torch_argmax(2)
...
The published results for bert_tiny_uncased on these datasets are 0.72 and 0.73, so our results of 0.65 and 0.66 after a single epoch are on track.
