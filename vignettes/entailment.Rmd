...
We can use `dataset_bert()` to process the train, matched, and mismatched datasets.

```{r datasets}
train_ds <- dataset_bert(
  x = dplyr::select(
    mnli_tibbles$train, 
    sentence1,
    sentence2
...
test_mismatched_ds <- dataset_bert(
  x = dplyr::select(
...
entailment_classifier <- torch::nn_module(
  ...
  initialize = function(model_name = "bert_tiny_uncased") {
    ...
    self$bert <- model_bert_pretrained(model_name)
