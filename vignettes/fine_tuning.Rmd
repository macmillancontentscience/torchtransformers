...
spine_with_pooling <- torch::nn_module(
  ...
  forward = function(x) {
    # BERT is built on attention, which expects inputs to have dimensions of
    # n_tokens by batch_size by embedding_size. Since our inputs are batch_size
    # by n_tokens by embedding_size, we transpose those first two dimensions.
    output <- self$bert(
    ...
    # token (length = n_tokens), example (length = batch_size), embeddings
    # (length = defined by the model) type.
    output <- output[1, , ]
